...
#define VL53L0X_DEV_NAME "vl53l0x"
...
static int vl53l0x_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (!indio_dev)
	{
		dev_err(&client->dev, "%s: allocation failed (ENOMEM)\n", __FUNCTION__);
		return -ENOMEM;
	}
	...
	{
		dev_err(&client->dev, "%s: i2c_check_functionality failed (EOPNOTSUPP)\n", __FUNCTION__);
		return -EOPNOTSUPP;
	}
	...
	int errno = devm_iio_device_register(&client->dev, indio_dev);
	
	if (errno != 0) dev_err(&client->dev, "%s: Device registration failed ", __FUNCTION__);
	else dev_dbg(&client->dev, "%s: Done\n", __FUNCTION__);
...
static const struct of_device_id st_vl53l0x_dt_match[] = {
	{ 
		...
		.data = VL53L0X_DEV_NAME,
	},
	{ }
};
MODULE_DEVICE_TABLE(of, st_vl53l0x_dt_match);

static const struct i2c_device_id st_vl53l0x_i2c_id_table[] = {
	{ VL53L0X_DEV_NAME },
	{},
};
MODULE_DEVICE_TABLE(i2c, st_vl53l0x_i2c_id_table);
...
static struct i2c_driver vl53l0x_driver = {
	...
	.probe = vl53l0x_probe,
	.id_table = st_vl53l0x_i2c_id_table,
};
